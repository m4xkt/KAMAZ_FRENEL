...
cv::Mat g_image;
const string g_windowName = "Fresnel Diffraction on a Slit";
...
int main() {
    ...
    // 4. Установка начальных позиций (должны совпадать с параметрами в п.1)
    cv::setTrackbarPos("Slit width [x0.01 mm]", g_windowName, 40);   // 0.5 мм
    cv::setTrackbarPos("Distance [x0.5 cm]",     g_windowName, 28);  // 15.0 см
    cv::setTrackbarPos("Brightness [x0.1]",      g_windowName,  9);  // 1.0x
    cv::setTrackbarPos("Zoom [x0.05]",           g_windowName, 16);  // 1.0x
    cv::setTrackbarPos("Resolution [um]",        g_windowName,  8);  // 5.0 мкм
